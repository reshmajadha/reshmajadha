...
public interface PharmacyRepo extends JpaRepository<Pharmacy, Integer> {
    @Query(value = " select p.pet_id,p.pet_name,p.pet_species,p.username,ph.medicine_name,ph.price,ph.quantity,\n" +
            "ph.medicine_expiry_date,ph.description \n" +
            "from pet p inner join\n" +
            "pharmacy_table ph on p.pet_id = ph.pet_id",nativeQuery = true)
